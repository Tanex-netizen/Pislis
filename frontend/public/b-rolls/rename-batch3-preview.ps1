# Rename remaining numeric b-roll files based on thumbnail content
...
Write-Host "Preview: Renaming remaining numeric files..." -ForegroundColor Cyan
...
foreach ($key in $map.Keys) {
  ...
  if ($matches.Count -eq 1) {
    Write-Host "Would rename:" $matches[0].Name "->" $map[$key] -ForegroundColor Green
    $successCount++
  } elseif ($matches.Count -eq 0) {
    Write-Host "No match for:" $key -ForegroundColor Yellow
  }
}

Write-Host "`nFound $successCount files to rename" -ForegroundColor Cyan
Write-Host "Run with -Apply parameter to execute renames" -ForegroundColor Cyan
